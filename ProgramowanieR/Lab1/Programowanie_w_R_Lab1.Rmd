...
wartości tekstowych `+` oraz `-`, reprezentujących dni, w których:
...
# Zadanie 5a
W ramach pewnego ubezpieczenia \(\boldsymbol{N}\) klientów płaci składkę wysokości \( K \) 
...
\hline
\textbf{Numer symulacji} & \multicolumn{5}{c|}{\textbf{Miesiąc symulacji}} \\
\cline{2-6}
 & 1 & 2 & 3 & \dots & \( T \) \\
\hline
1 & & & & & \\
...
\hline
\textbf{id} & \textbf{date} & \textbf{income} & \textbf{demographic} & \textbf{products} \\
\textit{Identyfikator klienta i umowy.} & \textit{Data dokonania wpisu.} & \textit{Roczny dochód klienta.} & \textit{Płeć, wiek i liczba dzieci.} & \textit{Posiadane obecnie produkty bankowe.} \\
\hline
...
\hline
\textbf{client\_id} & \textbf{agreement\_id} & \textbf{date} & \textbf{income} & \textbf{sex} & \textbf{age} & \textbf{child} & \textbf{dep} & \textbf{cre} & \textbf{mor} \\
\textit{Id klienta.} & \textit{Id umowy.} & \textit{Data.} & \textit{Dochód.} & \textit{Płeć.} & \textit{Wiek.} & \textit{Dzieci.} & \textit{Czy posiada depozyt?} & \textit{Czy posiada kredyt?} & \textit{Czy posiada kredyt hipoteczny?} \\
\hline
